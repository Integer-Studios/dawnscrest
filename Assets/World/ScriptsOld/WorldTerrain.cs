...
//			f -= hit.distance;
		...
		private void instantiateChunk(chunkIndex ci) {
			...
			c.name = "Chunk " + ci.x + "," + ci.z;
			c.AddComponent<Chunk> ();
			c.GetComponent<Chunk> ().initialize(new Vector3 (chunkSize * ci.x + (chunkSize / 2) + transform.position.x, 0, chunkSize * ci.z + (chunkSize / 2) + transform.position.z), c);
		}
